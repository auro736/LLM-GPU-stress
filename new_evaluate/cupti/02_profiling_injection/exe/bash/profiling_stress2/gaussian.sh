...
while [ "$(date +%s)" -lt "$end_time" ]; do
    ...
    env CUDA_INJECTION64_PATH=./libinjection.so ./test-apps/gpu-rodinia/bin/linux/cuda/gaussian -f ./test-apps/gpu-rodinia/gaussian/matrix1024.txt >> data/raw/stress/gaussian_$INJECTION_KERNEL_COUNT.txt &
    app_pid=$!
